...
async def add_to_cart(
    product_id, current_user: User, database: Session = Depends(db.get_db)
):
...
async def get_all_items(current_user: User, database) -> schema.ShowCart:
    user_info = database.query(User).filter(User.email == current_user.email).first()
    cart = database.query(Cart).filter(Cart.user_id == user_info.id).first()
    return cart


async def remove_cart_item(cart_item_id, current_user: User, database) -> None:
    user_info = database.query(User).filter(User.email == current_user.email).first()
